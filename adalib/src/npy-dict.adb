...
package body npy.dict is
   ...
   function Value (str : String) return Dictionary is
      ...
      barepat            : Pattern := '{' & Arb * baredict & '}';
      entrykey, entryval : VString;

      keypat   : Pattern := "'" & Arb * entrykey & "'" & Arb & ":";
      shapepat : Pattern := '(' & Arb * entryval & ')';

      valpat : Pattern := Arb * entryval & ",";

      newentry : dictentry;
   begin
      if Match (str, barepat) then
         if debug then
         ...
         curString := baredict;
         loop
            if Match (curString, keypat, "") then
               if debug then
                  Put ("Entry Key ");
                  Put (S (entrykey));
                  New_Line;
               ...
               if entrykey = "shape" then
                  if Match (curString, shapepat, "") then
                     if debug then
                        Put ("Found shape ");
                        Put_Line (S (entryval));
                     end if;
                  ...
                  if Match (curString, valpat, "") then
                     if debug then
                        Put ("Found value ");
                        Put_Line (S (entryval));
                     end if;
   ...
   begin
      loop
         if Match (shapestr, limpat, "") then
            f.Shape.Append (Integer'Value (S (limstr)));
            if Match (shapestr, termpat, "") then
               null;
            else
               return;
            end if;
